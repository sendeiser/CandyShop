              ...
              <tbody>
                {profiles.map((p, i) => (
                  <tr key={p.id} className={i % 2 === 0 ? '' : 'bg-[#faf6f5]/50'}>
                    ...
                    <td className="px-4 py-3 text-[#8d6e63]">{p.email}</td>
                    <td className="px-4 py-3 text-[#8d6e63]">{p.phone ?? '-'}</td>
                    ...
                    <td className="px-4 py-3 text-[#8d6e63]">{new Date(p.created_at).toLocaleDateString('es-AR')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
