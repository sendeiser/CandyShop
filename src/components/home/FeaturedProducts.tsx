    ...
    <section ref={sectionRef} className="relative overflow-hidden gradient-mesh px-6 py-16 sm:px-12 lg:px-20 reveal">
      <div className="pointer-events-none absolute inset-0">
        <div className="hidden sm:block">
          <div className="absolute right-12 top-10 h-10 w-10 rounded-full border-2 border-candy/20" style={{ animation: 'float-slow 6s ease-in-out 0s infinite' }} />
          <div className="absolute left-12 bottom-16 h-6 w-6 rounded-xl bg-candy/15 -rotate-12" style={{ animation: 'float-slow 5s ease-in-out 1.5s infinite' }} />
          <div className="absolute right-1/3 top-8 h-4 w-4 rounded-full bg-mint/20" style={{ animation: 'float-slow 4s ease-in-out 0.8s infinite' }} />
          <div className="absolute left-1/4 top-1/3 h-3 w-3 rounded-full bg-candy/15" style={{ animation: 'float-slow 3.5s ease-in-out 2.5s infinite' }} />
        </div>
      </div>

      <div className="relative mx-auto max-w-6xl">
        <div className="text-center">
          <h2 className="font-serif text-3xl font-bold text-chocolate sm:text-4xl">
            Los Mas Tentadores
          </h2>
              ...
              return (
                ...
                >
                  <div className="relative overflow-hidden rounded-t-2xl">
                    ...
                    <span className="absolute left-3 top-3 rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-candy shadow-sm">
                      {badge}
                    </span>
                  </div>
                  <div className="p-4">
                    <Link to={`/producto/${product.slug}`}>
                      <h3 className="font-sans text-base font-semibold text-chocolate transition-colors group-hover:text-candy">
                        {product.name}
                      </h3>
                    </Link>
                    <p className="mt-1 text-xl font-bold text-candy">
                    ...
                    <button
                      ...
                      className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl bg-candy py-2.5 text-sm font-semibold text-white transition-colors hover:bg-candy-dark active:scale-95"
                    >
